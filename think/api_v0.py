...
model_path_2 = "./luna-function.gguf"
def initialize_model(model_path= model_path, threads=6, context_size=4096, system_prompt="### System: You are Luna, a sexy lady trying to help out."):
    """
    ...
    if llm is None:
        with model_lock:
            if llm is None:  # Double-check lock pattern
                model_path = os.environ.get('MODEL_PATH', model_path_2)
                threads = int(os.environ.get('MODEL_THREADS', '6'))
                context_size = int(os.environ.get('CONTEXT_SIZE', '4096'))
                system_prompt = os.environ.get('SYSTEM_PROMPT', 
                                              "### System: You are Luna, a sexy lady trying to help out")
...
def chat():
    ...
    try:
        ...
        return jsonify({
            ...
            "model": os.path.basename(os.environ.get('MODEL_PATH', model_path))
        })
