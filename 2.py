# Check if user wants to debug the recording module
            if user_input.lower() == "debug":
                print("Running recording module diagnostics...")
                debug_hear_record_module()
                continuedef debug_hear_record_module():
    """Debug the hear.record module to identify issues"""
    print("\n=== Debugging Recording Module ===")
    ...
    try:
        ...
        # Try to import the hear.record module
        try:
            hear_record = importlib.import_module("hear.record")
            print("✅ hear.record module found")
            
            # Check if it has the expected functions
            if hasattr(hear_record, "record_audio"):
                print("✅ hear.record.record_audio function found")
            else:
                print("❌ hear.record.record_audio function NOT found")
                
            # Print module location
            print(f"Module location: {inspect.getfile(hear_record)}")
        except ImportError as e:
            print(f"❌ Cannot import hear.record module: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"❌ Could not list audio devices: {stderr}")
        except FileNotFoundError:
            print("❌ arecord command not found. Audio subsystem may not be ALSA.")
        except Exception as e:
            print(f"Error checking audio devices: {str(e)}")
            
        # Try to check if pulseaudio is running
        try:
            result = subprocess.run(
                ["pulseaudio", "--check"], 
                stdout=subprocess.PIPE, 
            ...
            print("ℹ️ PulseAudio command not found. May be using a different audio system.")
        except Exception as e:
            print(f"Error checking PulseAudio: {str(e)}")
    except Exception as e:
        print(f"Error checking audio subsystem: {str(e)}")
    
    # 3. Test if we can create files in the target directory
    print("\n=== Checking File Permissions ===")
    dirs_to_check = ["/tmp", os.path.expanduser("~"), os.getcwd()]
    ...
    # 4. Try direct recording using Python's built-in functionality if possible
    print("\n=== Testing Basic Audio Recording ===")
    try:
        try:
            # Try to import sounddevice if it's available
            import sounddevice as sd
            import scipy.io.wavfile as wav
            import numpy as np
            
            print("Testing direct recording with sounddevice...")
            test_file = "/tmp/test_recording_direct.wav"
            sample_rate = 44100
            duration = 1  # 1 second
            ...
            # Save the recording
            wav.write(test_file, sample_rate, data)
            
            if os.path.exists(test_file):
                file_size = os.path.getsize(test_file)
                print(f"✅ Direct recording successful. File size: {file_size} bytes")
                # Clean up
                os.remove(test_file)
            else:
                print("❌ Failed to create direct recording file")
                
        except ImportError:
            print("ℹ️ sounddevice and/or scipy not available for direct recording test")
        except Exception as e:
            print(f"Error during direct recording test: {str(e)}")
    except Exception as e:
        print(f"Error setting up direct recording test: {str(e)}")
    
    print("\n=== Recording Module Debugging Complete ===")
    print("Based on these results, check if the hear.record module is properly configured")
    print("and has the necessary permissions to access the audio device and write files.")
    print("=============================================\n")#!/usr/bin/env python3
import sys
import os
import subprocess
...
def start_recording(record_type="main", max_retries=3, retry_delay=1.0):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # First check if the directory exists, create if it doesn't
            output_dir = os.path.dirname(CURRENT_RECORDING_FILE)
            ...
            try:
                ...
                print(f"Warning: Cannot write to output directory: {str(e)}")
                # Try using /tmp as fallback
                CURRENT_RECORDING_FILE = f"/tmp/recording_{int(time.time())}.wav"
                print(f"Changed to fallback location: {CURRENT_RECORDING_FILE}")
            
            # Start the recording process
            cmd = ["python3", "-m", "hear.record", record_type, CURRENT_RECORDING_FILE, "noplay"]
            ...
            # Wait a moment to see if the process starts recording successfully
            time.sleep(0.5)
...
def main():
    ...
    print("Type 'debug' to run diagnostics on the recording module")
    print("=====================================\n")
